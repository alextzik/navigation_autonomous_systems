...
# Compatibility
def display_images_wrapper(img, prediction):    
    # Labels and scores
    p_labels = prediction[0]['labels'].cpu().detach().numpy()
    p_scores = prediction[0]['scores'].cpu().detach().numpy()

    score_mask = p_scores > 0.9

    p_labels = p_labels[score_mask]
    p_scores = p_scores[score_mask]

    # Boxes for visualization
    p_boxes = prediction[0]['boxes'].cpu().detach().numpy()
    p_boxes = np.array([[int(np.rint(x[1])), int(np.rint(x[0])), int(np.rint(x[3])), int(np.rint(x[2]))] for x in p_boxes]) # (y1, x1, y2, x2)

    p_boxes = p_boxes[score_mask]

    # Masks for visualization
    p_masks = prediction[0]['masks'].cpu().detach().numpy() > 0.5
    p_masks = p_masks[score_mask]
    p_masks = p_masks.transpose(2,3,0,1).squeeze(-1)
    
    class_names = ['BG', 'Vehicle']
    image = img.mul(255).permute(1, 2, 0).byte().numpy()
    
    fig = plt.figure(figsize=(16, 16))
    ax = plt.axes()

    display_instances(image, p_boxes, p_masks, p_labels,
                                class_names, p_scores, ax=ax)
